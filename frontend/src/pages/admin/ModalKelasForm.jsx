...
export const ModalKelasForm = ({ isOpen, onClose, onSuccess }) => {
  ...
  const fetchInitialData = async () => {
    try {
      ...
      setTahunAjaranOptions(res.data.tahun_ajaran)
      setSemesterOptions(res.data.semester)

      if (res.data.tahun_ajaran.length > 0) {
        setFormData(prev => ({ ...prev, tahun_ajaran: res.data.tahun_ajaran[0] }))
      }
      if (res.data.semester.length > 0) {
        setFormData(prev => ({ ...prev, semester: res.data.semester[0] }))
      }
    } catch {
  ...
  useEffect(() => {
    const fetchProdi = async () => {
      if (!formData.fakultas_id) { setProdiList([]); return }
      try {
  ...
  // Prodi + Semester → fetch Mata Kuliah
  useEffect(() => {
    const fetchMatkul = async () => {
      if (!formData.prodi_id || !formData.semester) { setMatkulList([]); return }
      try {
  ...
  const addKelasRow = () => {
    setFormData(prev => ({
      ...prev,
      kelas_list: [...prev.kelas_list, { nama_kelas: '', mata_kuliah_id: '' }]
    }))
  }
  ...
  const handleSubmit = async (e) => {
    ...
    formData.kelas_list.forEach(k => {
      if (!k.nama_kelas || !k.mata_kuliah_id) hasError = true
    })
    if (hasError) {
      ...
      <div className="p-2">
        ...
        <div className="flex items-center justify-center mb-8">
          <div className={`flex flex-col items-center ${step >= 1 ? 'text-brand-600 dark:text-brand-400' : 'text-gray-400'}`}>
            ...
            <span className="text-xs font-semibold uppercase tracking-wider">Struktur Kelas</span>
          </div>
          ...
          <div className={`flex flex-col items-center ${step >= 2 ? 'text-brand-600 dark:text-brand-400' : 'text-gray-400'}`}>
            ...
            <span className="text-xs font-semibold uppercase tracking-wider">Set Mata Kuliah</span>
          </div>
        </div>

        <form onSubmit={handleSubmit} className="space-y-6">

          {/* STEP 1 — Structure only: Fakultas, Prodi, Tahun Ajaran, Semester */}
          {step === 1 && (
            <div className="space-y-5 animate-in fade-in slide-in-from-right-4">
              ...
              <div className="p-4 rounded-xl bg-blue-50 dark:bg-blue-900/20 border border-blue-100 dark:border-blue-900/40">
                <p className="text-xs text-blue-700 dark:text-blue-300">
                  💡 <strong>Catatan:</strong> Dosen Pengajar dan Dosen PA di-assign terpisah melalui menu <strong>Assign Dosen</strong> setelah kelas dibuat.
                </p>
              </div>
          ...
          {/* STEP 2 — Only Nama Kelas + Mata Kuliah (NO Dosen) */}
          {step === 2 && (
